...
dynamic decodeApiResponse(
  ...
  if (body.isNotEmpty && contentType.contains('json')) {
    try {
      ...
      debugPrint('Haazir API invalid JSON (${response.statusCode}): $error');
    }
  }
  if (!successStatuses.contains(response.statusCode)) {
    ...
    throw ApiException(
      _messageForStatus(response.statusCode, backendMessage),
...
String _messageForStatus(int status, String? detail) {
  ...
  if (status == 409) {
    return detail ?? 'This request was accepted by another professional.';
  }
  if (status == 422) {
